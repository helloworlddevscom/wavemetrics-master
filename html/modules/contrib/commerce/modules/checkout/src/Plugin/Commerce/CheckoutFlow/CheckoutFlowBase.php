...
abstract class CheckoutFlowBase extends PluginBase implements CheckoutFlowInterface, ContainerFactoryPluginInterface {
  ...
  public function getPreviousStepId($step_id) {
    ...
    foreach($step_ids as $step_id_candidate) {
      if ($step_id == $step_id_candidate) {
        break;
      }
      if ($this->isStepVisible($step_id_candidate)) {
        $previous_step_id = $step_id_candidate;
      }
    }
  ...
  public function getNextStepId($step_id) {
    ...
    $next_step_id = NULL;
    foreach($step_ids as $step_id_candidate) {
      if ($subsequent && $this->isStepVisible($step_id_candidate)) {
        $next_step_id = $step_id_candidate;
        break;
      }
      if ($step_id == $step_id_candidate) {
        $subsequent = TRUE;
